...
CREATE TABLE IF NOT EXISTS settings (
        ...
        // Default settings
        await pool.query(`INSERT INTO settings (key, value) VALUES ('reviews_channel', '') ON CONFLICT (key) DO NOTHING;`);
...
// 7. VISUAL DB MANAGER
const isValidTable = (t) => ['users', 'products', 'expenses', 'faq', 'settings'].includes(t);
...
app.get('/api/reviews-channel', async (req, res) => {
    try {
        const result = await pool.query(`SELECT value FROM settings WHERE key='reviews_channel' LIMIT 1`);
