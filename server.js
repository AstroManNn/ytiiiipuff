...
const initDB = async () => {
    try {
        ...
        // В старых деплоях таблица `settings` могла уже существовать с другими именами колонок.
        // Поэтому аккуратно добавляем нужные колонки и (если возможно) переносим данные.
        ...
        if (!settingsCols.has('setting_key')) {
            await pool.query(`ALTER TABLE settings ADD COLUMN IF NOT EXISTS setting_key VARCHAR(100);`);

            // Миграция из самых частых legacy-колонок
            if (settingsCols.has('key')) {
                await pool.query(`UPDATE settings SET setting_key = COALESCE(setting_key, "key") WHERE setting_key IS NULL;`);
            }
            if (settingsCols.has('name')) {
                await pool.query(`UPDATE settings SET setting_key = COALESCE(setting_key, name) WHERE setting_key IS NULL;`);
            }
            if (settingsCols.has('setting')) {
                await pool.query(`UPDATE settings SET setting_key = COALESCE(setting_key, setting) WHERE setting_key IS NULL;`);
            }
        }

        if (!settingsCols.has('value')) {
            await pool.query(`ALTER TABLE settings ADD COLUMN IF NOT EXISTS value TEXT;`);
            if (settingsCols.has('setting_value')) {
                await pool.query(`UPDATE settings SET value = COALESCE(value, setting_value) WHERE value IS NULL;`);
            }
        }

        await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS settings_setting_key_uq ON settings(setting_key);`);

        // Ensure default setting exists
        await pool.query(`
            INSERT INTO settings (setting_key, value)
            VALUES ('reviews_channel_url','')
...
app.get('/api/reviews-channel', async (req, res) => {
    try {
        const r = await pool.query("SELECT value FROM settings WHERE setting_key='reviews_channel_url' LIMIT 1");
