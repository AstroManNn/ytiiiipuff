...
const ENABLE_BOT_POLLING = (process.env.ENABLE_BOT_POLLING || '').toLowerCase() === 'true';
const bot = new TelegramBot(process.env.BOT_TOKEN, ENABLE_BOT_POLLING ? { polling: true } : { polling: false });
// не даём ошибкам polling/webhook уронить процесс (особенно при деплое, когда Railway может поднять 2 инстанса)
bot.on("polling_error", (e) => console.error("❌ Telegram polling_error:", e && (e.message || e)));
bot.on("webhook_error", (e) => console.error("❌ Telegram webhook_error:", e && (e.message || e)));
...
CREATE TABLE IF NOT EXISTS cart_items (
        ...
        await pool.query(
            `INSERT INTO settings ("${global.__SETTINGS_KEY_COL}", "${global.__SETTINGS_VALUE_COL}")
             ...
             WHERE NOT EXISTS (
                 SELECT 1 FROM settings WHERE "${global.__SETTINGS_KEY_COL}" = $1::text
             );`,
            ['reviews_channel_url', '']
        );
...
const startServer = async () => {
    ...
    // graceful shutdown (Railway sends SIGTERM on redeploy/scale)
    const shutdown = () => {
        console.log('🟡 Received SIGTERM/SIGINT, shutting down gracefully...');
        server.close(() => {
            console.log('✅ HTTP server closed.');
            pool.end().catch(() => {});
            try { bot.stopPolling && bot.stopPolling(); } catch (e) {}
            process.exit(0);
        });
        // force-exit fallback
